...
$(busybox_prepare_full): $(BB_PATH)/busybox-full.config
	...
	@rm -rf $(bb_gen)/full
	@rm -f $(shell find -nowarn $(abspath $(call intermediates-dir-for,EXECUTABLES,busyboxsf)) -name "*.o")
	@mkdir -p $(@D)
...
$(busybox_prepare_minimal): $(BB_PATH)/busybox-minimal.config
	...
	@rm -f $(shell find -nowarn $(abspath $(call intermediates-dir-for,STATIC_LIBRARIES,libbusybox)) -name "*.o")
	@mkdir -p $(@D)
	@cat $^ > $@ && echo "CONFIG_CROSS_COMPILER_PREFIX=\"$(BUSYBOX_CROSS_COMPILER_PREFIX)\"" >> $@
	+make -C $(BB_PATH) prepare O=$(@D) $(BB_PREPARE_FLAGS)
